...
RSpec.describe 'projects' do
  ...
  describe 'projects:import', type: :rake do
    let(:area1) { create(:rma_area, name: 'Council 1') }
    let(:area2) { create(:rma_area, name: 'Council 2') }
    let(:csv_file) { Rails.root.join('tmp', 'projects.csv') }

    before do
      # create a CSV file with test data
      CSV.open(csv_file, 'wb') do |csv|
        csv << ['Project 1', 'ANC401I/000A/007A', area1.name]
        csv << ['Project 2', 'ANC401I/000A/008A', area1.name]
      end
    end
  ...
  describe 'projects:delete_all', type: :rake do
    ...
    it 'deletes all projects and their associated files' do
      # run the task
